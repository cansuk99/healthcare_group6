...
for features, features_selection_technique in zip([statistical_features, try2_features], ["manual statistical", "pseudo-random"]):
    ...
    for model_name in ["RandomForest", "LogisticRegression", "GradientBoosting"]:
        ...
        fig, axs = create_visualizations(y_test, y_pred, model_name, "predict_proba", features_selection_technique)
        ...
        fig, axs = create_visualizations(y_test, y_pred_predict_fn, model_name, "predict", features_selection_technique)
        plt.savefig(f"figures/manual_features_selection_results/{model_name}_predict_manual_statistical.png")
