...
for best_model_name in ["RandomForest", "LogisticRegression", "GradientBoosting"]:
    best_model = models[best_model_name]
    best_model.fit(X_train, y_train)

    best_thresh = float(results_df[results_df.Model == best_model_name]["Best_Threshold"])
    y_pred = (best_model.predict_proba(X_test)[:, 1] > best_thresh).astype(int)

    sns.set_theme(style="whitegrid", font_scale=1.1)
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(f"{best_model_name} Evaluation Dashboard – Manual Feature Selection, 'predict_proba'", fontsize=16, weight="bold")
