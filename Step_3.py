...
for idx, col in enumerate(key_numerical[:9]):  # Plot first 9
    ...
    # Add median line
    ax.axvline(df[col].median(), color='red', linestyle='--', linewidth=2, label='Median')
    ax.legend()
...
key_categorical = ['age', 'gender', 'race', 'admission_type_id', 
...
if 'readmitted_binary' not in df.columns:
    ...
    for col in key_numerical[:5]:  # Top 5 numerical features
        ...
        stat, pval = stats.ttest_ind(
            df[df['readmitted_binary']==1][col],
            df[df['readmitted_binary']==0][col]
        )
        ...
        print(f"    Significance: p={pval:.4f} {'***' if pval < 0.001 else '**' if pval < 0.01 else '*' if pval < 0.05 else 'ns'}")
    ...
    if 'num_medications' in df.columns:
        ...
        df['med_bins'] = pd.cut(df['num_medications'], bins=[0, 5, 10, 15, 20, 100], 
    ...
    if 'age' in df.columns:
        ...
        ax.set_ylabel('Readmission Rate (%)')
        ax.tick_params(axis='x', rotation=45)
        ax.axhline(df['readmitted_binary'].mean()*100, color='red', 
    ...
    if 'diag_1_category' in df.columns:
        ...
        diag_readmit.sort_values(ascending=True).plot(kind='barh', ax=ax, color='mediumseagreen')
        ax.set_title('Readmission Rate by Primary Diagnosis', fontweight='bold')
        ax.set_xlabel('Readmission Rate (%)')
        ax.set_ylabel('Diagnosis Category')
        ax.axvline(df['readmitted_binary'].mean()*100, color='red', 
                   linestyle='--', label='Overall Mean')
        ax.legend()
    
    plt.tight_layout()
    plt.savefig('03_bivariate_analysis.png', dpi=300, bbox_inches='tight')
...
if 'readmitted_binary' in corr_matrix.columns:
    ...
    sns.heatmap(corr_subset, annot=True, fmt='.2f', cmap='coolwarm', 
                ...
                ax=ax)
    ax.set_title('Correlation Matrix - Top Features with Readmission', 
...
if skewed_features:
    insights.append(f"3. Skewed features requiring transformation: {', '.join(skewed_features[:3])}")
...
eda_summary = {
    ...
    'Imbalance_Ratio': f"1:{imbalance_ratio:.1f}" if 'readmitted_binary' in df.columns else 'N/A'
}

summary_df = pd.DataFrame(list(eda_summary.items()), columns=['Metric', 'Value'])
summary_df.to_csv('03_eda_summary.csv', index=False)
print("\n✓ EDA summary saved as '03_eda_summary.csv'")

# Save insights
insights_df = pd.DataFrame({'Insight': insights})
insights_df.to_csv('03_key_insights.csv', index=False)
