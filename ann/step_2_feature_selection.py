...
df["discharge_postacute"] = (
    df["discharge_disposition_id"].isin(post_acute_codes).astype(int)
)
...
interaction_features.extend(
    ["meds_x_labs", "meds_x_visits", "stay_x_meds", "stay_x_labs", "labs_x_visits"]
)
...
cols_to_drop.extend(["encounter_id", "patient_nbr", "readmited_binary", "readmitted"])
...
df_encoded = pd.get_dummies(
    df, columns=categorical_to_encode, drop_first=True, dtype=int
)
...
feature_info.sort_values("Votes", ascending=False).to_csv(
    "../reports/04_selected_features.csv", index=False
)
